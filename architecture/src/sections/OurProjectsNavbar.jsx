...
const OurProjectsNavItems = ({ onClick = () => {}, className = "" }) => (
  <ul className={`nav-ul ${className}`}>
    <li className="nav-li">
      <Link to="/" className="nav-li_a text-sm md:text-base lg:text-lg hover:text-gray-0 transition-colors" onClick={onClick}>
        Home
      </Link>
    </li>
  </ul>
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/100 shadow-sm">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 md:px-6 lg:px-8 py-2 sm:py-6 md:py-2 lg:py-2">
        <div className="flex justify-between items-center h-16 sm:h-18 md:h-14 lg:h-14">
          ...
          <button
            onClick={toggleMenu}
            className="text-black hover:text-neutral-600 focus:outline-none sm:hidden flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 rounded-lg bg-white hover:bg-gray-200 transition-colors"
